...
public static String addNewReportToDB(int month1, int year1) {
	...
	try {
		stmt = mysqlConnection.conn.prepareStatement("INSERT INTO SubscribersStatusReport VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
	...
	try (PreparedStatement stmt = mysqlConnection.conn.prepareStatement(query)) {
		...
		try (ResultSet rs = stmt.executeQuery()) {
			if (rs.next()) {
				...
				statusReportData = month + ", " + year + ", " + totalActiveSubscribers + ", " + totalFrozenSubscribers + ", " + gotFrozeSubscribers + ", "
						+ unfrozedSubscribers + ", "+ newSubscribers + ", " + totalSubscribers;
			}
		}
	} catch (SQLException e) {
		e.printStackTrace();
